(function () {
  ...
  function checkBedTime() {
    ...
    for (var j = 0; j < goToBedTime.length; j++) {
      ...
      goToBedTime[goToBedTime.length - 1 - j].innerHTML =
        startTimeArr[0] + ":" + startTimeArr[1] + " " + startTimeArr[2];
    }
  ...
  function calculateTime(hours, min, dayHalf, change) {
    ...
    //returning the clock to the desired format
    if (newH === 0) {
      hours = hourBase;
    } else {
      hours = newH;
